...
class ConfigValidator:
    ...
    def validate_integer(
        cls, value: str, default: int, min_val: int = None, max_val: int = None
    ) -> int:
    ...
    def load_config(cls) -> AppConfig:
        ...
        database_config = DatabaseConfig(
            ...
            max_pool_size=cls.validate_integer(
                env_vars["MONGO_MAX_POOL_SIZE"], 10, 1, 100
            ),
            min_pool_size=cls.validate_integer(
                env_vars["MONGO_MIN_POOL_SIZE"], 2, 1, 50
            ),
            max_idle_time_ms=cls.validate_integer(
                env_vars["MONGO_MAX_IDLE_TIME_MS"], 30000, 1000, 300000
            ),
            server_selection_timeout_ms=cls.validate_integer(
                env_vars["MONGO_SERVER_SELECTION_TIMEOUT_MS"], 5000, 1000, 30000
            ),
            connect_timeout_ms=cls.validate_integer(
                env_vars["MONGO_CONNECT_TIMEOUT_MS"], 10000, 1000, 60000
            ),
            socket_timeout_ms=cls.validate_integer(
                env_vars["MONGO_SOCKET_TIMEOUT_MS"], 45000, 1000, 120000
            ),
            retry_writes=cls.validate_boolean(env_vars["MONGO_RETRY_WRITES"], True),
            write_concern=cls.validate_integer(
                env_vars["MONGO_WRITE_CONCERN"], 1, 1, 5
            ),
        )
        ...
        rate_limit_config = RateLimitConfig(
            ...
            monitoring_rate_limit=cls.validate_rate_limit(
                env_vars["MONITORING_RATE_LIMIT"]
            ),
        )
        ...
        logging_config = LoggingConfig(
            ...
            max_log_file_size=cls.validate_integer(
                env_vars["MAX_LOG_FILE_SIZE"], 10485760, 1024, 104857600
            ),
        ...
        logger.info(
            f"Rate limits: webhook={app_config.rate_limit.webhook_rate_limit}, api={app_config.rate_limit.api_rate_limit}"
        )
...
def validate_required_config() -> bool:
    ...
    try:
        ...
        if not config.security.hmac_secret_key:
            raise ConfigurationError(
                "HMAC_SECRET_KEY is required", config_key="HMAC_SECRET_KEY"
            )
        ...
        raise ConfigurationError(
            f"Configuration validation failed: {str(e)}", config_key="validation_error"
        ) from e
